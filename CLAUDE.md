...
- `id` con la forma `T-K7M3XQ`: `T-` y un código de seis caracteres del alfabeto `23456789ABCDEFGHJKMNPQRSTUVWXYZ` (sin `0`, `1`, `I`, `L` ni `O`, que se confunden), generado al azar al crear la tarea y único en el servidor. **No es correlativo** (decidido el 16 de septiembre de 2026: un número que sube cuenta las tareas y ordena lo que no debe ordenarse). Las tareas creadas antes conservan su número como código (`T-0042`): un id ya citado en un hilo, en la actividad o en un commit sigue valiendo. Por eso la forma que se acepta al leer es `T-` y de cuatro a ocho caracteres de `[0-9A-Z]`. Por dentro la fila sigue teniendo un número (`tareas.id`, `AUTOINCREMENT`, que **nunca se reutiliza** y que es el que enlazan las demás tablas); el código va en `tareas.codigo`, `TEXT NOT NULL UNIQUE`, y es lo único que ven la web, el Markdown y los agentes. **El id es global, no por proyecto**: identifica una tarea en cualquier repositorio y un commit ya escrito nunca queda ambiguo. Además, `titulo`, `descripcion`, `estado`, `orden` (posición dentro de su columna), `padre` (opcional, para tareas hijas).
...
- **`en marcha`**: un terminal la ha tomado con `tomar_tarea` y todavía no ha escrito el comentario que cierra esa fase. La web la enseña con su edad: `en marcha 12 min`. Ver «Fases paradas».
- **`parada`**: la fase lleva en marcha más del umbral (`FASE_PARADA_HORAS`, seis por defecto). Lo más probable es que el subagente haya muerto sin cerrarla. Es un aviso al humano, que la libera desde la ficha; no frena a nadie ni la libera sola. Ver «Fases paradas».
...
- **En `done`, comentar es pedir otra iteración.** Un solo botón, «Comentar»: escribe el comentario y devuelve la tarea a `doing` en la misma transacción. Se probó ofrecer también un «Comentar» que solo dejara constancia y se quitó el 16 de septiembre de 2026: si el resultado vale, se finaliza; si se escribe algo, es porque falta algo. Es la vuelta atrás de antes, dicha en el chat: el servidor sigue sin mover nada hacia atrás por su cuenta. El cuadro lo dice debajo del botón: «Comentar devuelve la tarea al agente para otra iteración».
- **Un comentario vacío no se guarda** (`comentario_vacio`): el navegador lo frena con `required` y el servidor lo rechaza igual, porque en `done` un comentario vacío devolvería la tarea a `doing` sin decir por qué.
- **Las iteraciones se cuentan, no se guardan.** Iteración 1 es la primera ejecución; cada transición `done → doing` empieza una más. La ficha dice «Iteración 3» en la cabecera, junto a la edad en el estado, y el hilo pinta un separador «Iteración 2», «Iteración 3»… en el punto de cada vuelta, con la fecha de la transición. Sale de la tabla `transiciones`, así que las tareas anteriores a esa tabla cuentan solo desde que existe.
...
- **La ficha enseña las dos direcciones** (decidido el 17 de septiembre de 2026): la fila «Depende de» de siempre y, debajo, una fila «Bloquea a» con las tareas que declaran depender de esta, cada una con su id enlazado y su etiqueta de estado; sin ninguna, «ninguna». Es lo que dice cuántas tareas hay detrás de desbloquear esta. Sale de `dependientesDe`, que ya existía para la confirmación de borrado. El MCP no lo expone: al agente le basta `esperando`.
...
- **`clave`**: de dos a ocho caracteres, mayúsculas y cifras, empieza por letra (`DEFAULT`, `WEB`, `API2`). Es lo que va en las URLs de la web, en el chip de las tarjetas y en el frontmatter. Se fija al crear el proyecto y no se cambia (`clave_invalida` si no cumple la forma, `clave_repetida` si ya existe).
...
- **`tareas.proyecto_id`** y **`terminales.proyecto_id`**, `INTEGER NOT NULL REFERENCES proyectos(id)`. La migración crea el proyecto por defecto con id 1 y cuelga de él todo lo existente: para quien tiene un solo repositorio nada cambia. Se llamó `PRI` «Principal» hasta el 16 de septiembre de 2026; desde la migración 013 es `DEFAULT` «Default», y la misma migración renombra el existente si aún se llama `PRI`. El índice `tareas_por_columna` pasa a ser `(proyecto_id, estado, orden)`.
...
- **`proyectos.color`**, `TEXT NOT NULL` con `CHECK` sobre los ocho colores de usuario (`src/db/colores.ts`). Es el color de las siglas del proyecto en su chip. Sin elegir, el alta asigna el menos usado entre los proyectos, como con los usuarios; se cambia en la edición del proyecto. La migración 013 la añade y reparte colores a los existentes por su id, `(id - 1) % 8`. Cambiar el color no sube la revisión.
...
- **Una tarea no cambia de proyecto desde la web**: nace en el que se está viendo y la edición no lo ofrece. La regla de la base sigue existiendo por si algún día hace falta desde el CLI: solo en `backlog` y solo si no tiene padre, ni hijas, ni dependencias en ningún sentido (`no_cambia_de_proyecto`). Sus asignaciones de terminal se ponen a nulo, porque un terminal es de un solo proyecto.
...
- **El CLI y el primer arranque** crean terminales en `DEFAULT` salvo que se dé la clave: `npm run cli -- crear-terminal <usuario> <nombre> <cuenta> [clave]`. Hay también `npm run cli -- crear-proyecto <clave> <nombre>`.

### En el Markdown

- **El frontmatter lleva `proyecto: DEFAULT`** justo debajo de `id`. La línea de índice no lo lleva: el agente solo ve tareas de su proyecto.
...
- **El proyecto va en la URL**: `/p/WEB/tareas`, `/p/WEB/tareas/kanban`, `/p/WEB/tareas/nueva`, `/p/WEB/funcionalidades` y `/p/WEB/informes` son las vistas de siempre acotadas a ese proyecto. Los enlaces se comparten y se abren en varias pestañas. Una clave que no existe es 404.
- **El proyecto que se está viendo se recuerda** (decidido el 16 de septiembre de 2026): cada página acotada escribe la cookie `proyecto` con su clave (`SameSite=Lax`, un año, sin firmar: es una preferencia, no una credencial). Las entradas de la navegación (Tareas, Funcionalidades, Informes, Nueva tarea) apuntan al proyecto recordado; sin cookie, al proyecto por defecto (`DEFAULT`). Nunca a la vista cruzada por sí solas.
- **La vista de todos los proyectos se elige a mano**: la opción «Todos los proyectos» del selector escribe `todos` en la cookie y lleva a las rutas sin prefijo; mientras está puesta, la navegación apunta a ellas. Elegir un proyecto en el selector la sustituye. Una cookie con una clave que ya no existe se trata como ausente.
...
- **Selector de proyecto** en lo alto de la barra lateral, debajo del nombre de la aplicación: un desplegable con cada proyecto por su clave y nombre y, al final, «Todos los proyectos». Preseleccionado el de la URL o, fuera de una página acotada, el recordado. Cambiarlo lleva a la misma vista en el proyecto elegido y lo recuerda. Las entradas Lista, Kanban y Funcionalidades de la navegación apuntan al proyecto de la URL actual, o a la vista cruzada si no hay ninguno.
...
- **`GET /agentes`, `GET /agentes/nuevo`, `POST /agentes`, `GET /agentes/:id/editar`, `POST /agentes/:id/editar`, `GET /agentes/:id/borrar`, `POST /agentes/:id/borrar`**, entrada «Agentes» en el bloque «Configuración», entre Proyectos y Terminales. Lista: nombre, descripción, modelo, terminal (o «cualquiera»), fases asignadas (cuántas fases de tareas abiertas lo llevan; una tarea con el mismo papel en las dos cuenta dos), «Creado por». Alta y edición en página aparte, como los proyectos: nombre (solo en el alta), descripción, instrucciones en un `<textarea rows="18">` monoespaciado, modelo como desplegable, terminal como desplegable con «cualquiera». Frase de propósito: «Un agente es un papel: quién es, cómo trabaja y con qué modelo. Las tareas se lo asignan por fase.».
...
- **La ficha de una tarea** enseña el agente en las filas Análisis y Ejecución de las propiedades: `revisor · sonnet@portatil-ana`, con el nombre enlazado a la ficha del agente.
- **La ficha de un agente**, `GET /agentes/:id` (decidido el 17 de septiembre de 2026): un papel solo se puede juzgar por lo que ha hecho. Cabecera con el nombre, la descripción como frase de propósito y los botones «Editar» y «Borrar» a la derecha; el bloque de propiedades con modelo, terminal (o «cualquiera»), fases asignadas y «Creado por»; las instrucciones renderizadas con markdown-it; y una tabla «Tareas trabajadas» con las últimas veinte tareas que lo tienen asignado en alguna fase, de la más reciente a la más antigua: id enlazado, título, fase (Análisis o Ejecución), etiqueta de estado, tokens de esa fase abreviados y cuántas veces fue devuelta (`done → doing` en `transiciones`). Una tarea con el papel en las dos fases sale en dos filas. Sin tareas, «Todavía no ha trabajado ninguna tarea.». Se calcula sobre las tareas que lo tienen asignado ahora: borrar el agente pone las referencias a nulo y su historial desaparece con él, y `consumo` no guarda el agente; si algún día hace falta el histórico de verdad, sería una columna en `consumo`. El nombre del agente en la lista de agentes enlaza aquí, y la edición sigue en `/agentes/:id/editar`.
...
- **La edad** se enseña como `12 min`, `5 h` o `3 d` (`formatos.ts`), en texto suave, en la tarjeta del kanban (a la derecha del id) y en la lista, en la columna Actualizada, que pasa a llamarse «En columna» y a enseñar la edad en vez de la fecha; la fecha completa va en el `title`. En la ficha, la cabecera añade «desde hace 3 d» en texto suave junto a las etiquetas.
...
Si el subagente muere a medias, la fase se queda «en marcha» para siempre: ningún terminal la vuelve a tomar y la única salida era borrar la tarea, que se lleva el hilo. Decidido el 17 de septiembre de 2026: se guarda cuándo se tomó la fase, se avisa cuando lleva demasiado, y el humano la libera con un botón. No se libera sola: una ejecución larga de verdad puede pasar del umbral, y soltarla con el subagente vivo lanzaría dos sobre el mismo trabajo.

- **Columna `tareas.en_marcha_desde`**, `TEXT`, anulable. La escribe `tomar_tarea` con la hora actual, en el mismo sitio que `en_marcha_terminal_id`; y se pone a nulo donde ese campo se pone a nulo: el comentario que cierra la fase, el borrado del terminal y la liberación. Va en la misma migración que nada más: es un `ALTER TABLE ... ADD COLUMN`.
- **La marca `en marcha` lleva edad**: `en marcha 12 min`, con `edad` de `formatos.ts`, en tarjeta, fila, ficha y bandeja, en texto suave como la edad en columna. En el frontmatter la marca sigue siendo `en marcha` a secas: el MCP no expone edades.
- **Marca derivada `parada`**, naranja como `sin terminal`: hay `en_marcha_desde` y han pasado más de `FASE_PARADA_HORAS` horas. Sale donde salen las demás, frontmatter incluido (`marcas: [en marcha, parada]`). Se calcula en `marcasDe`, que recibe `en_marcha_desde` y el umbral.
- **`FASE_PARADA_HORAS`**, variable de entorno del servidor, entera, seis por defecto. Va a `config.ts` como las demás.
- **`POST /tareas/T-0042/liberar`** pone `en_marcha_terminal_id` y `en_marcha_desde` a nulo, escribe un `comentario` con autor `servidor` que dice «Fase liberada por humano:ana. La tenía en marcha portatil-ana desde el 2026-09-17T08:05:00Z.», deja rastro `liberar_fase` con detalle `analisis, portatil-ana` y sube la revisión: la tarea vuelve a estar disponible y `novedades` la ofrece. Solo vale con una fase en marcha (`sin_fase_en_marcha`). No cambia el estado ni `estado_desde`: una ejecución liberada sigue en `doing` y la retoma quien la tome. En la ficha es un botón «Liberar la fase» junto a la marca, dentro del panel de propiedades, solo cuando hay una fase en marcha; con la marca `parada`, el botón dice además desde cuándo.
- **El bucle no cambia.** `tomar_tarea` sobre una fase liberada funciona como sobre una fase sin tomar. Si el subagente original seguía vivo y escribe después, su comentario entra igual: el hilo lo enseña y el humano lo ve.
...
- **El cuadro de comentar** cierra el hilo, pegado al último mensaje, como en un chat: un `textarea` y el botón «Comentar». En `done` el mismo botón pide otra iteración y el cuadro lo avisa debajo. En `finished` no hay cuadro. Ver «El hilo como chat».
...
- **Sin desplegables** (decidido el 16 de septiembre de 2026): los filtros por proyecto, estado, terminal, marca y funcionalidad se quitan de la lista y del tablero. El proyecto ya lo acota la URL, el estado es la columna, y el resto no se usaba. Los parámetros siguen funcionando en la URL para quien los escriba, pero no se pintan. Quedan el conmutador Lista | Tablero, los tres conmutadores rápidos, la búsqueda y «Agrupar por funcionalidad».
- **Tres conmutadores** a la izquierda de la búsqueda de la lista y del tablero, como enlaces con aspecto de botón: «Espera por ti» (tareas `bloqueadas`, con `análisis listo` o en `done`: las mismas que llevan la etiqueta), «Agente trabajando» y «Sin terminal». Van en el parámetro `?rapido=espera|en-marcha|sin-terminal`, uno cada vez; el activo lleva `aria-current="true"` y pulsarlo lo quita. Se combinan con los desplegables y con el ámbito del proyecto.
...
- **Se busca desde cualquier pantalla** (decidido el 17 de septiembre de 2026): la barra lateral lleva, debajo del selector de proyecto, un `<form method="get">` con un `<input type="search" name="q" placeholder="Buscar tareas">` y un botón «Buscar» que el cliente esconde, como el «Ir» del selector. Envía a la lista del proyecto recordado (`/p/WEB/tareas?q=`) o, con «Todos los proyectos» puesto, a `/tareas?q=`. Sin JavaScript, sin ruta nueva: es la misma `?q=` de la lista. La misma limitación de `LIKE` sobre `lower()`.

**Carriles por funcionalidad**

- **Agrupado por defecto** (decidido el 16 de septiembre de 2026): una funcionalidad agrupa tareas y así se ve. El tablero arranca agrupado por funcionalidad; el conmutador dice «Por columnas» y quita la agrupación con `?agrupar=no`. La lista también se agrupa: cada funcionalidad es una sección con su cabecera (id, título enlazado, etiqueta de estado, progreso) y una tabla de sus partes con la columna Estado (la etiqueta) delante del título, ordenadas por columna y orden; después la sección «Sueltas» con los grupos por columna de siempre para lo que no cuelga de ninguna funcionalidad. Las funcionalidades no salen como fila: son las cabeceras. El mismo orden de funcionalidades que en el tablero. `?agrupar=no` devuelve la lista por columnas.
- **El conmutador** es un enlace en la fila de filtros que dice a dónde va: «Por columnas» (`?agrupar=no`) cuando está agrupado, «Agrupar por funcionalidad» (sin parámetro) cuando no. `?agrupar=funcionalidad` sigue valiendo como sinónimo del defecto. Es un parámetro de la URL, así que el refresco en vivo lo conserva; la búsqueda lo reemite como campo oculto.
...
- **Debajo, el ritmo**: tareas que llegaron a `done`, como tabla de dos columnas (fecha, tareas) con la barra de progreso de la tarjeta como gráfico, con el máximo como `max`. Sin librería de gráficos. Con periodo de 7 o 30 días es **por día, los últimos catorce**, con los días sin tareas a cero para que la fila exista; con 90 o «todo» es por semana, como hasta ahora (decidido el 17 de septiembre de 2026: con pocas semanas de datos, tres barras semanales no dicen nada). Son dos funciones en `informes.ts`, `ritmoPorDia` y `ritmo`, cada una una sentencia SQL; la ruta elige cuál según el periodo.
...
| `POST /tareas/T-0042/liberar` | Suelta la fase que un terminal tiene en marcha. Ver «Fases paradas» |
| `GET /agentes/:id` | La ficha de un agente: su papel y lo que ha trabajado. Ver «Agentes › En la web» |
...
| `GET /proyectos`, `GET /proyectos/nuevo`, `POST /proyectos`, `GET /proyectos/:id/editar`, `POST /proyectos/:id/editar`, `GET /proyectos/:id/borrar`, `POST /proyectos/:id/borrar` | Proyectos: lista, alta en página aparte, edición y borrado con confirmación en página aparte |
...
La web es el puesto de mando de una persona que dirige agentes: lo que le importa es saber qué espera por ella y en qué punto del ciclo está cada tarea. Rediseñada el 14 de septiembre de 2026 con esa idea, y confirmada el 17 de septiembre de 2026 frente a dos alternativas dibujadas en un lienzo de Claude Design (una editorial con serif y una densa y oscura): se queda esta, «Puesto de mando». Ese lienzo es la referencia visual; un cambio de aspecto se retoca allí antes de tocar `estilos.ts`. Todo el estilo sale de `src/web/estilos.ts`; los componentes reutilizables (etiquetas, chips, cabecera de página, propiedades, pasos del ciclo) de `src/web/componentes.ts`; el esqueleto de página de `src/web/plantilla.ts`; las palabras de `src/web/vocabulario.ts`. Ningún estilo en línea en las plantillas.
...
| `done → doing` | Comentar (en Hechas pide otra iteración) |
...
- Marcas: `bloqueada` rojo, `sin terminal` naranja, `en marcha` morado, `análisis listo` rosa, `esperando` naranja, `sobre presupuesto` naranja, `parada` naranja.
...
- **La etiqueta de turno** no es una marca guardada: es la que la tarjeta, la fila y la ficha pintan en `--turno` (`insignia turno`, texto `--sobre-turno` sobre fondo `--turno`) cuando la tarea está `done` o lleva `bloqueada` o `análisis listo`: exactamente lo que cuenta el contador de pendientes y lo que filtra el conmutador rápido «Espera por ti». **Su texto es el verbo de la bandeja**: «Contesta» con `bloqueada`, «Aprueba» con `análisis listo`, «Revisa» en `done` (decidido el 17 de septiembre de 2026; antes decía «Espera por ti»). Así la tarjeta dice qué te toca sin descifrar marcas, y la bandeja y el tablero hablan igual. `backlog` no la lleva: la columna entera es del humano y marcarla una a una sería ruido. Es lo único que va en ese color además del contador de la bandeja y el paso actual del ciclo.
...
- `pasosDelCiclo(tarea)` pinta el ciclo como `<ol class="ciclo">` con cinco `<li>`: una línea de cinco puntos unidos por un filete fino (`span.punto` y el nombre de la columna debajo, centrados), no cinco cajas. El dueño («tú», «el agente», «nadie») va en el `title` del `<li>`, no en pantalla; en el paso actual (`aria-current="step"`) el punto se pinta en `--turno` o `--acento` con un halo de `--turno-fondo`/`--fondo-hover` y debajo del nombre va la frase de qué pasa ahora como píldora (`span.paso-ahora`, fondo `--turno-fondo`): «Termina de definirla y pásala a preparadas», «El agente de análisis la está estudiando», «Espera tu respuesta a P1», «Espera tu aprobación del análisis», «El agente la está ejecutando», «Revisa el resultado», «Cerrada». Los pasos pasados llevan el punto relleno en `--texto-suave` y el nombre en `--texto-suave`; el actual va en `--turno` si el turno es del humano y en `--acento` si es del agente, y su `title` dice a quién le toca de verdad («tú» en una Preparada bloqueada, aunque la columna sea del agente); en los demás pasos el `title` es el dueño de la columna. Las clases `pasado`, `turno`, `agente` y `omitido` se conservan. Una pregunta salta el paso En curso; una funcionalidad dice «Revisa las partes» en Preparada y «Las partes se están trabajando» en En curso.
...
- **Tablero**: columnas sin fondo; la cabecera de cada columna es la etiqueta de estado con el nombre de la columna y el contador, y el dueño («la defines tú», «la trabaja el agente», «la revisas tú») en una línea de texto suave debajo del rótulo (`p.dueno-columna`), no dentro de él. Tarjetas con borde `--borde`, fondo `--fondo`, sombra suave al pasar y al arrastrar; una tarea que espera por el humano lleva un filete de 3 px en `--turno` en el borde izquierdo. Cada tarjeta lleva **una etiqueta como máximo**: la de turno con su verbo si le toca al humano, y si no la primera marca que tenga (`parada`, `en marcha`, `sin terminal`, `esperando`, `sobre presupuesto`, en ese orden: una fase parada lleva también `en marcha` y lo que importa es la parada; el tipo `funcionalidad` o `pregunta` se pinta siempre, es otra cosa). Anatomía: id a la izquierda y edad a la derecha con la etiqueta entre ambos, título, la barra de progreso cuando hay hijas, y una última línea con los agentes a la izquierda como chips pequeños de inicial (`span.agentes`: análisis y ejecución, en ese orden; un modelo sin asignar es un círculo hueco con «?», y el `title` de cada chip lleva `modelo@terminal`) y los tokens a la derecha. El terminal ya no se lee en la tarjeta: está en la ficha. Agrupado por funcionalidad, cada franja lleva su cabecera de ancho completo sobre `--fondo-lateral` y las columnas debajo; ver «Un tablero que se lee de un vistazo».
- **Ficha**: migas `DEFAULT › Tareas › T-0042`, título, y debajo las etiquetas: estado, la de turno con su verbo si toca, el tipo, y en texto suave «Iteración 2 · desde hace 3 h» (la iteración solo cuando n > 1); las acciones hacia delante a la derecha (Pasar a preparadas, Aprobar ejecución, Finalizar). Debajo de la cabecera, **el ciclo** (`pasosDelCiclo`) a todo lo ancho, como línea de puntos. La fila Estado de propiedades desaparece: estado, edad e iteración ya están en la cabecera, y las marcas también (los tests y el botón «Liberar la fase» se apoyan en ellas). Debajo, las preguntas abiertas con su formulario. Después el bloque de **propiedades**: filas de dos columnas con el nombre en `--texto-suave` y el valor al lado: Proyecto, Tipo, Rama (solo si la tiene, en monoespaciada), Análisis, Ejecución, Autoejecución, Padre, Depende de, Bloquea a, Orden, Creada (chip de quien la creó, o el terminal si fue una propuesta, y la fecha), Revisión. Después Descripción, Hijas (el título del bloque dice `Hijas 2/5` con la misma `<progress class="progreso">` de la tarjeta y debajo, en texto suave, el desglose «2 hechas · 2 en curso · 1 con pregunta abierta», con los nombres del vocabulario y nunca el nombre interno de la marca; cada hija con su etiqueta de estado), Consumo (fases con nombre legible), Hilo con el cuadro de comentar al final, Actividad. La fila Estado añade «Iteración 2» cuando hay más de una. «Devolver a por definir», **Editar** y **Borrar** van al final como `<details>`, porque son excepcionales; pedir otra iteración no lo es y va en el cuadro de comentar. En ancho, propiedades, consumo y `<details>` forman el panel de la derecha; ver «La ficha como vista de incidencia».
- **Bandeja**: título «Bandeja», frase «Lo que espera por ti, de todos los proyectos.» y cuatro bloques con un verbo como título y el contador al lado: «Contesta» (preguntas sin contestar), «Aprueba» (análisis y descomposiciones por aprobar), «Revisa» (resultados en Hechas) y «Define» (más de siete días por definir). Debajo de cada verbo, una línea que dice qué es el bloque. **Cada pendiente es una sola tarjeta** (`article.item`, con filete de 3 px en `--turno`): una cabecera propia (`div.cabecera-item`, separada del cuerpo por un borde `--borde`) con chip de proyecto, id, título en negrita, el chip del agente que escribió lo que hay que atender y la edad a la derecha; y el cuerpo con lo que toca (la pregunta con su formulario, el análisis con «Aprobar ejecución» o «Aprobar descomposición», el resultado). Ya no hay una línea suelta encima de la tarjeta. En «Revisa», el pie es una sola fila: el campo de comentar de una línea («Escribe al agente para pedir otra iteración…»), el botón «Pedir otra iteración» (que exige texto en el campo) y «Finalizar» como principal. En «Define», solo la cabecera. Un bloque vacío dice «Nada pendiente.». La tarjeta de pregunta y la de resultado son los mismos componentes que en el hilo de la ficha: nada se pinta dos veces con dos plantillas.
- **Hilo**: se lee como una conversación, sin cajas. Cada mensaje (`article.comentario`) es un bloque separado del anterior por una regla fina `--borde` arriba, con cabecera de chip del autor en negrita, etiqueta del tipo (salvo `comentario`, que no lleva: es lo corriente), `P<n>` cuando toca y la fecha a la derecha; debajo el cuerpo renderizado. Los mensajes del humano (`article.comentario.humano`) van sobre `--fondo-lateral` con radio de tarjeta y sin regla, sangrando 16 px a cada lado para que la banda sobresalga del texto. El separador de iteración conserva su forma. El formulario de respuesta va dentro del mensaje de la pregunta abierta, con las opciones como tarjetas seleccionables y la recomendada marcada. El cuadro de comentar es fijo al fondo de la ventana mientras se hace scroll por el hilo (`position: sticky; bottom: 0` sobre `--fondo`, con regla superior), para escribir sin volver abajo.
- **Nueva tarea** y **Editar**: una columna, etiquetas encima de los campos, en este orden: Título, Tipo, Descripción, Rama, Funcionalidad de la que es parte, Dependencias, Presupuesto, Autoejecución, y las fases. **El tipo por defecto es `pregunta`**: lo más frecuente es preguntarle algo al agente. **El proyecto no se elige**: la tarea nace en el proyecto que se está viendo (el de la URL o el recordado) y el formulario lo enseña como texto con su chip, sin desplegable ni campo; tampoco se cambia al editar. `POST /tareas` sin prefijo crea en el recordado. Decidido el 16 de septiembre de 2026. Análisis y Ejecución como dos tarjetas lado a lado a partir de 48 rem.
- **Terminales**: encima de la tabla, la franja **«Ahora mismo»** (decidido el 17 de septiembre de 2026): una tarjeta por terminal no revocado, con el nombre, el chip de su proyecto, si está conectado y desde cuándo, y debajo sus fases en marcha, una por línea: el id enlazado, el título, la fase con nombre legible, el modelo y `en marcha 12 min` (en `--peligro` si lleva la marca `parada`). Un terminal sin nada en marcha dice «En reposo» y la edad de su última conexión. Es la única pantalla que contesta «¿está pasando algo o se ha muerto el bucle?». Sale de una sola consulta para toda la franja, nunca una por terminal, y se refresca con el mismo intervalo que la telemetría de la tabla. Después, la tabla de siempre: chip del dueño, columna «Creado por» y, si está revocado, «Revocado por». El uso disponible se enseña por ventana como barra fina (`data-nivel` de 0 a 10, en rojo con 2 o menos) con el porcentaje disponible y la hora de reinicio; `resets_at` se acepta en segundos desde la época o en ISO 8601. «Revocar» va como enlace rojo discreto en la fila, no como botón: con diez columnas el botón no cabía. La columna «Agentes» lleva el número en un formulario mínimo a `POST /terminales/:id/agentes` (un `<input type="number" min="1">` y un botón «Guardar»), y el alta pide el mismo valor con 1 preseleccionado.
...
| `liberar_fase` | tarea | `analisis, portatil-ana`: la fase liberada y el terminal que la tenía |
...
| `FASE_PARADA_HORAS` | Horas que una fase puede llevar en marcha antes de recibir la marca `parada`. Ver «Fases paradas» | `6` |
...
| `npm run cli -- crear-terminal <usuario> <nombre> <cuenta> [clave]` | Crea un terminal en ese proyecto (`DEFAULT` si no se da) e imprime su token una sola vez |
...
1. **El número siguiente sale de `git tag -l | sort -V | tail -1`**, nunca de memoria: otra sesión puede haber publicado entre medias (pasó el 17 de septiembre de 2026: una «0.1.7» etiquetada después de la 0.2.1, corregida como 0.2.2). **La versión va en seis sitios a la vez**: `server/package.json`, `plugin/.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `server/Dockerfile` (`org.opencontainers.image.version`), `docker-compose.yml` de la raíz (`VERSION:-X.Y.Z`) y el README (los ejemplos de «Publicar una versión» y la tabla del `.env`); `server/compose.yaml` la cita en un comentario. Se comprueba con `git grep` de la versión anterior.
