...
| `VERSION` | no | Etiqueta de la imagen, `0.1.4` por defecto. Para actualizar, súbela y vuelve a `docker compose up -d`; las migraciones de la base de datos corren solas al arrancar. |
...
docker buildx build --platform linux/amd64,linux/arm64 -t xinux87/mcp-tareas-server:0.1.4 --push .
```

Desde el clon, `IMAGEN=xinux87/mcp-tareas-server:0.1.4 docker compose up` la descarga en vez de
