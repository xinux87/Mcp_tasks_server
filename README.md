...
tomado alguien y desde cuándo, `parada` si esa fase lleva demasiado sin cerrarse y conviene liberarla desde la ficha, `esperando` si depende de otra tarea que aún no está hecha), y con `autoejecucion`
...
| `VERSION` | no | Etiqueta de la imagen, `0.2.2` por defecto. Para actualizar, súbela y vuelve a `docker compose up -d`; las migraciones de la base de datos corren solas al arrancar. |
...
docker buildx build --platform linux/amd64,linux/arm64 -t xinux87/mcp-tareas-server:0.2.2 --push .
```

Desde el clon, `IMAGEN=xinux87/mcp-tareas-server:0.2.2 docker compose up` la descarga en vez de
